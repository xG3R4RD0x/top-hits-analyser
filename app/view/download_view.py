...
class DownloadView(BaseView):
    ...
    def setup_ui(self):
        # Título de la vista
        self.view_label = ttk.Label(self, text="Descarga de Canciones", font=("Helvetica", 14))
    ...
    def create_download_list(self):
        ...
        scrollbar = ttk.Scrollbar(self.download_frame, orient="vertical", command=self.canvas.yview)
    ...
    def add_download_item(self, song_name, artist, status="Pendiente"):
        ...
        label = ttk.Label(item_frame, text=f"{song_name} - {artist}", width=30, anchor="w")
    ...
    def start_download(self):
        ...
        # Crear un hilo para cada descarga simulada
        self.stop_simulation = False
        threads = []
        
        for row_id in self.download_rows:
            thread = threading.Thread(
                target=self.simulate_download_progress, 
                args=(row_id,)
            )
            thread.daemon = True
            ...
            # Pequeña pausa para que no inicien todas a la vez
            time.sleep(0.1)
